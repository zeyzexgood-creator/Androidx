...
package dev.mutwakil.androidide.javac.services.partial

import dev.mutwakil.androidide.javac.services.compiler.JavacFlowListener
import dev.mutwakil.androidide.javac.services.util.ReparserUtils
import dev.mutwakil.androidide.javac.services.visitors.FindAnonymousVisitor
import dev.mutwakil.androidide.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = dev.mutwakil.androidide.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      dev.mutwakil.androidide.javac.services.NBParserFactory.instance(context)
        as dev.mutwakil.androidide.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = dev.mutwakil.androidide.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is dev.mutwakil.androidide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      dev.mutwakil.androidide.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
