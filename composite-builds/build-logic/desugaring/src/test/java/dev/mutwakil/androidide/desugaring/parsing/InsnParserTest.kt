package dev.mutwakil.androidide.desugaring.parsing
...
import dev.mutwakil.androidide.desugaring.internal.parsing.InsnParser.ParseException
...
class InsnParserTest {
  ...
  fun `test simple instruction parse`() {
    ...
    assertThat(insn.toClass).isEqualTo(
      "dev.mutwakil.androidide.desugaring.sample.java.io.DesugaredInputStream")
  ...
  """.trimIndent()
    ...
    insns[1].also { insn ->
      ...
      assertThat(insn.toOpcode).isEqualTo(MethodOpcode.INVOKESTATIC)
      assertThat(insn.toClass).isEqualTo(
        "dev.mutwakil.androidide.desugaring.sample.java.util.DesugaredMath")
