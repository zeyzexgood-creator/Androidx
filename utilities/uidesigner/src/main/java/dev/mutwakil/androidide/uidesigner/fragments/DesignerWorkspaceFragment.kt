...
import dev.mutwakil.androidide.fragments.BaseFragment
...
import dev.mutwakil.androidide.uidesigner.databinding.FragmentDesignerWorkspaceBinding
import dev.mutwakil.androidide.uidesigner.drag.WidgetDragListener
import dev.mutwakil.androidide.uidesigner.drag.WidgetTouchListener
import dev.mutwakil.androidide.uidesigner.drawable.UiViewLayeredForeground
import dev.mutwakil.androidide.uidesigner.models.CommonUiView
import dev.mutwakil.androidide.uidesigner.models.PlaceholderView
import dev.mutwakil.androidide.uidesigner.models.RootWorkspaceView
import dev.mutwakil.androidide.uidesigner.models.UiViewGroup
import dev.mutwakil.androidide.uidesigner.undo.UndoManager
import dev.mutwakil.androidide.uidesigner.utils.UiLayoutInflater
import dev.mutwakil.androidide.uidesigner.utils.applyPreview
import dev.mutwakil.androidide.uidesigner.utils.bgDesignerView
import dev.mutwakil.androidide.uidesigner.utils.layeredForeground
import dev.mutwakil.androidide.uidesigner.viewmodel.M3ComponentState
...
class DesignerWorkspaceFragment : BaseFragment() {
  ...
  private fun createM3ComponentFromState(m3Component: M3ComponentState): IView? {
    return try {
      ...
      val layoutFile = dev.mutwakil.androidide.inflater.internal.LayoutFile(File(""), "")
      val iView =
          when (view) {
            is MaterialButton -> {
              // Use ViewImpl instead of checking the actual view type
              dev.mutwakil.androidide.inflater.internal.ViewImpl(layoutFile, m3Component.type, view)
            }
            is MaterialCardView -> {
              dev.mutwakil.androidide.inflater.internal.ViewGroupImpl(layoutFile, m3Component.type, view)
            }
            is MaterialTextView -> {
              dev.mutwakil.androidide.inflater.internal.ViewImpl(layoutFile, m3Component.type, view)
            }
        ...
        try {
          val namespace =
              if (attrName.startsWith("app:")) {
                dev.mutwakil.androidide.inflater.INamespace.APP
              } else {
                dev.mutwakil.androidide.inflater.INamespace.ANDROID
              }
          val attr =
              dev.mutwakil.androidide.inflater.internal.AttributeImpl(
                  namespace = namespace,
