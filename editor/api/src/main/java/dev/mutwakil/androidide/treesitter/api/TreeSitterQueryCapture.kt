...
class TreeSitterQueryCapture @JvmOverloads internal constructor(
  ...
  companion object {
    ...
    fun obtain(node: TSNode?, index: Int): TreeSitterQueryCapture {
      return _root_ide_package_.dev.mutwakil.androidide.treesitter.api.obtainFromPool<TreeSitterQueryCapture>()
          .apply {
        this.node = node
        this.index = index
      }
  ...
  override fun recycle() {
    (this.node as? dev.mutwakil.androidide.treesitter.api.TreeSitterNode?)?.apply {
      if (!isRecycled) {
        recycle()
      }
    }
